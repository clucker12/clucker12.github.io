---
title: "[프로그래머스] 퍼즐 조각 채우기 (BFS + 회전 매칭)"
date: 2025-12-01
categories: [Algorithm, Programmers]
tags: [BFS, 시뮬레이션, 도형매칭, 회전, Java]
layout: single
author_profile: true
read_time: true
share: true
...
# 🧩 프로그래머스 - 퍼즐 조각 채우기 (Java, BFS + 회전 매칭)

## 📌 문제 개요

게임 보드에서 **빈칸(0)** 을 찾고, 테이블에서 **퍼즐 조각(1)** 을 찾아  
회전을 포함한 모든 형태를 비교하여 빈칸을 채울 수 있는 최대 칸 수를 구하는 문제.
...
1. **BFS** 로 게임 보드와 테이블에서 각각 도형(connected component) 추출
2. 도형을 **정규화(normalize) → 회전(rotate) → 정렬(sort)**
3. 모든 조각에 대해 빈칸과 **4번 회전하여 모양이 일치하는지 비교**
4. 일치하면 해당 조각을 사용하고 정답 += 도형 크기

---

## 🧩 사용된 주요 알고리즘 & 자료구조

- **BFS** : 도형을 구성하는 연결된 칸을 탐색
- **리스트(List<int[]> )** : 도형의 상대 좌표 저장
- **좌표 정규화(normalize)** : 도형을 (0,0)을 기준으로 이동
- **회전 변환** : 90도 회전 (x, y) → (y, -x)
- **정렬 & 비교** : 좌표 리스트를 정렬하여 모양 비교
...
각 칸을 최대 한 번 방문 → O(N^2)

✔ 2) 도형 회전 및 비교

- 조각 개수 최대: 약 6~10개
- 회전 4번 × 좌표 정렬 O(K log K)
- 비교: O(K)

전체적으로
O(N² + P × K log K)
(문제의 N이 작기 때문에 충분히 빠름)
...
1. extractShapes()
   보드 전체를 돌며 BFS로 빈칸/조각을 리스트 형태로 추출.

2. bfs()
   도형 내 모든 칸을 상대 좌표로 저장
   → 이후 비교를 위해 normalize + sort 수행.

3. normalize()
   도형을 왼쪽 상단(0,0) 기준으로 이동하여
   회전 후에도 항상 같은 좌표계 사용 가능.

4. rotate()
   90도씩 회전 (x,y) → (y, -x)
   회전 후 normalize + sort로 표준 형태로 유지.

5. match()
   회전 4번 시도해 blank와 block 모양이 동일하면 true.

---

## 💡 느낀점 (2025-12-01)

퍼즐 조각 문제는 얼핏 보면 단순 구현 같지만,
도형 정규화 → 회전 → 매칭 로직 이 정확하지 않으면
테스트 케이스에서 쉽게 틀리는 문제였다.

특히 normalize 과정이 핵심이며,
“도형을 좌표 기준으로 비교하기 위해서는
절대 좌표가 아닌 상대 좌표로 표현해야 한다”
는 개념을 다시 생각하게 만든 문제였다.

회전, 정렬, 좌표 이동을 체계적으로 조합하는 게 중요했고
구현 난도가 꽤 있는 시뮬레이션 문제였다고 느꼈다.
